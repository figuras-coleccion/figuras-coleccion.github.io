...
export default function SettingsPage() {
  ...
  const allCodes = activeAlbum.allStickersOrdered
  const missingCount = allCodes.reduce((total, code) => total + (stickers[code]?.owned ? 0 : 1), 0)
  ...
  const handleCompleteAlbum = async () => {
    ...
    try {
      ...
      updates[`${statsPath}/lastBulkCompleteAt`] = now
      updates[`${statsPath}/lastActivityAt`] = now
      await update(ref(db), updates)

      window.setTimeout(() => {
        window.location.assign(buildAppPath(ALBUM_ROUTE, '?completed=1'))
      }, 900)
    } catch (error) {
  ...
  const handleAlbumChange = async (nextAlbumId) => {
    const saved = await saveToCloud()
    if (!saved) return
    await changeAlbum(nextAlbumId)
  ...
  return (
    <div className="settings-page">
      ...
      <section className="settings-menu-card" aria-label="Opciones de configuración">
        ...
        <button type="button" onClick={() => setAlbumPickerOpen(true)}><span className="settings-menu-icon green">⇄</span><span><strong>Cambiar de álbum</strong><small>Actual: {activeAlbum.shortTitle}</small></span><i>›</i></button>
      ...
      {albumPickerOpen ? <AlbumPickerModal albums={availableAlbums} activeAlbumId={activeAlbumId} userId={user?.id} onCancel={() => setAlbumPickerOpen(false)} onConfirm={handleAlbumChange} /> : null}
      {figuritasImportOpen && activeAlbumId === DEFAULT_ALBUM_ID ? (
        <FiguritasImportModal
          onClose={() => setFiguritasImportOpen(false)}
          onFinish={() => {
            setFiguritasImportOpen(false)
